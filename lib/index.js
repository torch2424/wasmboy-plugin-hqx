...
import wasmModule from '../pkg/wasmboy_plugin_hqx_bg.wasm';
...
export default function (passedUpscaleFactor) {
  ...
  const task = async () => {
    const response = await rustWasmInit(wasmModule);
    console.log('lib init response:', response);
    // response.greet();
  };
  task();
  ...
  return {
    ...
    graphics: (imageDataArray) => {
      // console.log('graphics hook', imageDataArray);
    }
  }
}
